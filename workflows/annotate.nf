// WORKFLOW file for only running microbeannotator (the "annotate" workflow)
...
def helpMessage() {
  ...
  Usage for annotate workflow:
    ...
    Database arguments:
      --pangene_seqs                Path to the folder with protein sequences from pangenomes of choice. Only needed for full workflow.
      --uniref_seqs			        Path to the folder with protein sequences from Uniref90. Only needed for full workflow.
...
/*
========================================================================================
    Define channels for fasta inputs
========================================================================================
*/
// The channel should look like this: [SRR493366, [/my/data/SRR493366.fa]]

if (params.process_rna && params.mtx_protein_seqs){
	Channel.fromFilePairs( [params.mtx_protein_seqs + '/**{fa,faa,fasta}'], checkIfExists:true, size: 1 ).set{ ch_mtx_fa_input }
}

if (params.process_dna && params.mgx_protein_seqs){
	Channel.fromFilePairs( [params.mgx_protein_seqs + '/**{fa,faa,fasta}'], checkIfExists:true, size: 1 ).set{ ch_mgx_fa_input }
}
...
if (params.process_rna){
    ANNOT_MTX(params.microbeannotator_db, ch_mtx_fa_input)
}
if (params.process_dna){
    ANNOT_MGX(params.microbeannotator_db, ch_mgx_fa_input)
}
